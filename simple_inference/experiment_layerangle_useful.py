#!/usr/bin/env python3
"""
Layer-wise angle analysis for Mamba cache mixing strategy.
...
layers based on their cosine similarity.

The key idea: For each layer, we identify dimensions where the two caches have
the LOWEST cosine similarity (most different angles), and use the "good" cache
only for those bottom-k% positions.
"""
...
def inference_layerangle(
    ...
    """
    Hybrid cache inference using layer-wise angle (cosine similarity) mixing strategy.
    
    Strategy:
    - Fast path: few-shot + doc2 (less context, faster)
    - Slow path: few-shot + doc1 + doc2 (full context, slower but better)
    - Hybrid: For each layer, mix caches based on bottom-k% cosine similarity
    ...
    Args:
        ...
        top_k_percent: Percentage of dimensions with lowest cosine similarity to use "good" cache (default: 10%)
    ...
    if verbose:
        print("=" * 70)
        print("HYBRID CACHE INFERENCE (LAYER-WISE ANGLE)")
        print("=" * 70)
    ...
    # ========== Create hybrid cache ==========
    if verbose:
        print("\nCreating hybrid cache (layer-wise angle)...")
    ...
    # ========== Vectorized cache mixing with cosine similarity ==========
    ...
    # For each layer, select bottom-k% dimensions with LOWEST cosine similarity
    # (most different angles)
    # Shape: [num_layers, batch * d_inner]
    num_layers_actual = cosine_sim.shape[0]
    batch_size = cosine_sim.shape[1]
    d_inner = cosine_sim.shape[2]
    
    cosine_flat = cosine_sim.view(num_layers_actual, -1)
    
    # Compute k dimensions per layer
    k_dims = max(1, int(cosine_flat.shape[1] * top_k_percent / 100.0))
    
    # Get bottom-k dimension indices for each layer (lowest cosine similarity)
    # Shape: [num_layers, k_dims]
    _, bottom_dim_indices = torch.topk(cosine_flat, k_dims, dim=1, largest=False)
    
    # Create masks for dimensions
    # Shape: [num_layers, batch * d_inner]
    dim_masks = torch.zeros_like(cosine_flat, dtype=torch.bool)
    dim_masks.scatter_(1, bottom_dim_indices, True)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Layer-wise angle (cosine similarity) hybrid cache experiment")
    parser.add_argument('top_k_percent', type=float, help='Percentage of dimensions with lowest cosine similarity to use slow cache')
    ...
    if args.test_mode:
        ...
        print("=" * 70)
        print("LAYER-WISE ANGLE HYBRID CACHE")
        print("=" * 70)
        ...
        output_file = os.path.join(args.output_dir, f"layerangle{k_str}_useful_{timestamp}.csv")
