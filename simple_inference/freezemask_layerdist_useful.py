#!/usr/bin/env python3
"""
Freeze mask generation for layer-wise distance strategy.
...
def get_mask_layerdist(cache_fast, cache_slow, top_k_percent):
    """
    Get mask for layer-wise distance strategy.
    ...
    # Compute L2 distance per dimension
    diff_vectors = ssm_fast_stack - ssm_slow_stack
    distance_per_dim = torch.norm(diff_vectors, p=2, dim=-1)  # [num_layers, batch, d_inner]
    
    # Layer-wise top-k selection
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate freeze mask for layer-wise distance strategy")
    parser.add_argument('top_k_percent', type=float, help='Percentage of dimensions to use slow cache')
    ...
    print("=" * 70)
    print("GENERATING FREEZE MASK (LAYER-WISE DISTANCE)")
    print("=" * 70)
    ...
    for item in tqdm(sample_dataset, desc="Processing"):
        ...
        try:
            ...
            # Get mask
            mask = get_mask_layerdist(cache_fast, cache_slow, args.top_k_percent)
    ...
    output_file = os.path.join(args.output_dir, f"mask_layerdist{k_str}_useful.npy")
