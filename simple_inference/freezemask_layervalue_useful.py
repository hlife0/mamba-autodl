#!/usr/bin/env python3
"""
Freeze mask generation for layer-wise value strategy.
...
def get_mask_layervalue(cache_fast, cache_slow, top_k_percent):
    """
    Get mask for layer-wise value strategy.
    ...
    # Layer-wise value-level top-k selection
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate freeze mask for layer-wise value strategy")
    ...
    print("=" * 70)
    print("GENERATING FREEZE MASK (LAYER-WISE VALUE)")
    print("=" * 70)
    ...
    for item in tqdm(sample_dataset, desc="Processing"):
        ...
        try:
            ...
            # Get mask
            mask = get_mask_layervalue(cache_fast, cache_slow, args.top_k_percent)
    ...
    output_file = os.path.join(args.output_dir, f"mask_layervalue{k_str}_useful.npy")
