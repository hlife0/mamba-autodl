#!/usr/bin/env python3
"""
Freeze mask generation for layer-wise angle (cosine similarity) strategy.
...
def get_mask_layerangle(cache_fast, cache_slow, top_k_percent):
    """
    Get mask for layer-wise angle (cosine similarity) strategy.
    ...
    cosine_sim = (ssm_fast_norm * ssm_slow_norm).sum(dim=-1)  # [num_layers, batch, d_inner]
    
    # Layer-wise bottom-k selection (lowest cosine similarity)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate freeze mask for layer-wise angle strategy")
    ...
    print("=" * 70)
    print("GENERATING FREEZE MASK (LAYER-WISE ANGLE)")
    print("=" * 70)
    ...
    for item in tqdm(sample_dataset, desc="Processing"):
        ...
        try:
            ...
            # Get mask
            mask = get_mask_layerangle(cache_fast, cache_slow, args.top_k_percent)
    ...
    output_file = os.path.join(args.output_dir, f"mask_layerangle{k_str}_useful.npy")
