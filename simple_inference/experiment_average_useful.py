#!/usr/bin/env python3
"""
Average cache baseline: Average the caches from (fewshot + doc1) and (fewshot + doc2).
"""
...
def inference_average(model, tokenizer, question, doc1, doc2, max_new_tokens=30, device="cuda", verbose=False):
    ...
    cache_avg.seqlen_offset = cache_1.seqlen_offset  # Use same offset (they should be equal)
    
    # ========== Vectorized cache averaging ==========
    ...
    # Average the caches
    conv_avg_stack = (conv_1_stack + conv_2_stack) / 2.0
    ssm_avg_stack = (ssm_1_stack + ssm_2_stack) / 2.0
    ...
    if verbose:
        print(f"  Averaged {num_layers} layers")
    
    # ========== Prefill question with averaged cache ==========
    if verbose:
    ...
    if verbose:
        print(f"  Cache offset after question: {cache_avg.seqlen_offset}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Average cache baseline experiment")
    ...
    if args.test_mode:
        ...
        output_file = os.path.join(args.output_dir, f"average_useful_{timestamp}.csv")
