#!/usr/bin/env python3
"""
Layer-wise distribution analysis for Mamba cache mixing strategy.

This script implements a hybrid caching strategy that mixes "fast but poor" cache
(with less context) and "slow but good" cache (with full context) at different
layers based on their state differences.

The key idea: For each layer, we identify positions where the two caches differ
the most, and use the "good" cache only for those top-k% positions, while using
the "fast" cache for the rest.
"""
...
def inference_layerdist(
    ...
    """
    Hybrid cache inference using layer-wise distribution mixing strategy.
    
    Strategy:
    - Fast path: few-shot + doc2 (less context, faster)
    - Slow path: few-shot + doc1 + doc2 (full context, slower but better)
    - Hybrid: For each layer, mix caches based on top-k% difference positions
    ...
    Args:
        ...
        top_k_percent: Percentage of positions to use "good" cache (default: 10%)
    ...
    # Compute L2 distance between fast and slow versions
    # Shape: [num_layers, batch, d_inner, d_state]
    diff_vectors = ssm_fast_stack - ssm_slow_stack
    
    # Compute L2 norm along d_state dimension for each (layer, batch, dimension)
    # Shape: [num_layers, batch, d_inner]
    distance_per_dim = torch.norm(diff_vectors, p=2, dim=-1)
    
    # For each layer, select top-k% dimensions with largest distances
    # Shape: [num_layers, batch * d_inner]
    num_layers_actual = distance_per_dim.shape[0]
    batch_size = distance_per_dim.shape[1]
    d_inner = distance_per_dim.shape[2]
    
    distance_flat = distance_per_dim.view(num_layers_actual, -1)
    
    # Compute k dimensions per layer
    k_dims = max(1, int(distance_flat.shape[1] * top_k_percent / 100.0))
    
    # Get top-k dimension indices for each layer
    # Shape: [num_layers, k_dims]
    _, top_dim_indices = torch.topk(distance_flat, k_dims, dim=1)
    
    # Create masks for dimensions
    # Shape: [num_layers, batch * d_inner]
    dim_masks = torch.zeros_like(distance_flat, dtype=torch.bool)
    dim_masks.scatter_(1, top_dim_indices, True)
    
    # Reshape back to [num_layers, batch, d_inner]
    dim_masks = dim_masks.view(num_layers_actual, batch_size, d_inner)
    ...
    # Apply masks: use slow cache for top-k dimensions, fast cache for others
    ssm_hybrid_stack = torch.where(full_masks, ssm_slow_stack, ssm_fast_stack)
    
    # Store results in cache_hybrid
    for layer_idx in range(num_layers):
        conv_fast, _ = cache_fast.key_value_memory_dict[layer_idx]
        conv_hybrid = conv_fast.clone()
        ssm_hybrid = ssm_hybrid_stack[layer_idx]
        cache_hybrid.key_value_memory_dict[layer_idx] = (conv_hybrid, ssm_hybrid)
    
    total_dimensions = distance_flat.numel()
    total_mixed_dims = k_dims * num_layers
    total_elements = ssm_fast_stack.numel()
    total_mixed_elements = total_mixed_dims * d_state
    
    if verbose:
        print(f"  Mixed {total_mixed_dims}/{total_dimensions} dimensions ({top_k_percent:.2f}%)")
        print(f"  Total elements mixed: {total_mixed_elements}/{total_elements} ({total_mixed_elements/total_elements*100:.2f}%)")
    ...
    cache_hybrid, first_token = prefill_with_cache(
        model, input_ids_question, cache_hybrid, device
    )
    if verbose:
    ...
    for _ in range(max_new_tokens - 1):
        cache_hybrid, next_token = decode_with_cache(
            model, current_token, cache_hybrid, device
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Mamba hybrid cache inference experiment")
    parser.add_argument('top_k_percent', type=float, help='Select top k percent dimensions for hybrid cache')
    ...
    parser.add_argument('--output_dir', type=str, default='./simple_inference/experiments', help='Output directory for results')
    parser.add_argument('--test_mode', action='store_true', help='Run in test mode with single example')
    ...
    if args.test_mode:
        ...
        output_file = os.path.join(args.output_dir, f"layerdist{int(args.top_k_percent)}_useful_{timestamp}.csv")
        ...
        print(f"Strategy: Hybrid cache (top-k={args.top_k_percent}%)")
        print()
