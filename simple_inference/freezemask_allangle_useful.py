#!/usr/bin/env python3
"""
Freeze mask generation for global angle (cosine similarity) strategy.
...
def get_mask_allangle(cache_fast, cache_slow, top_k_percent):
    """
    Get mask for global angle (cosine similarity) strategy.
    ...
    # Global bottom-k selection (lowest cosine similarity)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate freeze mask for global angle strategy")
    ...
    print("=" * 70)
    print("GENERATING FREEZE MASK (GLOBAL ANGLE)")
    print("=" * 70)
    ...
    for item in tqdm(sample_dataset, desc="Processing"):
        ...
        try:
            ...
            # Get mask
            mask = get_mask_allangle(cache_fast, cache_slow, args.top_k_percent)
    ...
    output_file = os.path.join(args.output_dir, f"mask_allangle{k_str}_useful.npy")
