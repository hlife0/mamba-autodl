#!/usr/bin/env python3
"""
All-value analysis for Mamba cache mixing strategy.

This script implements a hybrid caching strategy that treats each individual value
across ALL (layer, dimension, hidden_state) as independent, selecting global top-k% values.

The key idea: Flatten all values across all layers and select the global top-k% values
with largest differences to use the "good" cache.
"""
...
def inference_allvalue(
    ...
    """
    Hybrid cache inference using global value-level mixing strategy.
    
    Strategy:
    - Fast path: few-shot + doc2 (less context, faster)
    - Slow path: few-shot + doc1 + doc2 (full context, slower but better)
    - Hybrid: Select global top-k% individual values across all layers
    ...
    Args:
        ...
        top_k_percent: Percentage of ALL values to use "good" cache (default: 10%)
    ...
    if verbose:
        print("=" * 70)
        print("HYBRID CACHE INFERENCE (GLOBAL VALUE-LEVEL)")
        print("=" * 70)
    ...
    # ========== Create hybrid cache ==========
    if verbose:
        print("\nCreating hybrid cache (global value-level)...")
    ...
    # ========== Vectorized cache mixing with GLOBAL VALUE-LEVEL top-k ==========
    ...
    # Flatten ALL values across ALL layers for global top-k selection
    # Shape: [num_layers * batch * d_inner * d_state]
    diff_global_flat = diff_values.view(-1)
    
    # Compute k values globally
    total_values = diff_global_flat.shape[0]
    k_values_global = max(1, int(total_values * top_k_percent / 100.0))
    
    if verbose:
        print(f"  Total values: {total_values}")
        print(f"  Selecting top {k_values_global} values ({top_k_percent}%)")
    
    # Get global top-k value indices
    # Shape: [k_values_global]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Global value-level hybrid cache experiment")
    parser.add_argument('top_k_percent', type=float, help='Percentage of all values to use slow cache (global top-k)')
    ...
    if args.test_mode:
        ...
        print("=" * 70)
        print("GLOBAL VALUE-LEVEL HYBRID CACHE")
        print("=" * 70)
        ...
        output_file = os.path.join(args.output_dir, f"allvalue{k_str}_useful_{timestamp}.csv")
