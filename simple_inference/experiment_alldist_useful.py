#!/usr/bin/env python3
"""
All-dimension distribution analysis for Mamba cache mixing strategy.

This script implements a hybrid caching strategy that mixes "fast but poor" cache
(with less context) and "slow but good" cache (with full context) across ALL layers
based on their state differences.

The key idea: Consider ALL dimensions across ALL layers, compute distances, and select
the global top-k% dimensions to use the "good" cache.
"""
...
def inference_alldist(
    ...
    """
    Hybrid cache inference using global (all-layer) distribution mixing strategy.
    
    Strategy:
    - Fast path: few-shot + doc2 (less context, faster)
    - Slow path: few-shot + doc1 + doc2 (full context, slower but better)
    - Hybrid: Consider ALL dimensions across ALL layers, select global top-k%
    ...
    Args:
        ...
        top_k_percent: Percentage of ALL dimensions to use "good" cache (default: 10%)
    ...
    if verbose:
        print("=" * 70)
        print("HYBRID CACHE INFERENCE (GLOBAL TOP-K)")
        print("=" * 70)
    ...
    # ========== Create hybrid cache ==========
    if verbose:
        print("\nCreating hybrid cache (global top-k)...")
    ...
    # ========== Vectorized cache mixing with GLOBAL top-k ==========
    ...
    # Flatten ALL dimensions across ALL layers for global top-k selection
    # Shape: [num_layers * batch * d_inner]
    num_layers_actual = distance_per_dim.shape[0]
    batch_size = distance_per_dim.shape[1]
    d_inner = distance_per_dim.shape[2]
    
    distance_global_flat = distance_per_dim.view(-1)
    
    # Compute k dimensions globally
    total_dims = distance_global_flat.shape[0]
    k_dims_global = max(1, int(total_dims * top_k_percent / 100.0))
    
    if verbose:
        print(f"  Total dimensions: {total_dims}")
        print(f"  Selecting top {k_dims_global} dimensions ({top_k_percent}%)")
    
    # Get global top-k dimension indices
    # Shape: [k_dims_global]
    _, top_global_indices = torch.topk(distance_global_flat, k_dims_global, dim=0)
    
    # Create global mask
    # Shape: [num_layers * batch * d_inner]
    global_mask = torch.zeros_like(distance_global_flat, dtype=torch.bool)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="All-dimension distribution hybrid cache experiment")
    parser.add_argument('top_k_percent', type=float, help='Percentage of dimensions to use slow cache (global top-k)')
    ...
    if args.test_mode:
        ...
        print("=" * 70)
        print("ALL-DIMENSION DISTRIBUTION HYBRID CACHE")
        print("=" * 70)
        ...
        output_file = os.path.join(args.output_dir, f"alldist{k_str}_useful_{timestamp}.csv")
